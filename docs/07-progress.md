...
- 第一件偏热身货，第二件继续小额试水，降低新玩家一上来遇到高价重仓或强陷阱的挫败感。
- 教学拍品仍避免重复，并优先匹配当日热点候选。
- 上台日志增加节奏保护提示，让玩家知道前两件适合练手。
- 存档 key 升到 `kelly-king-save-v11`，当前阶段标记更新到 8.3。
...
- 按用户建议重写教程结构，去掉混杂的开发输出式叙述，改为面向用户的操作流程
- 新结构围绕“先规划 → 再编排 → 再执行”：基础想法、策划专家、架构专家、项目经理、开发者逐步执行、验证记录
- 增加“用户提示词目标和实例”部分，按生成策划案、设计架构、编排任务、执行任务、修 Bug、体验优化分类
- 增加“AI 交付物清单”，明确策划、架构、编排、执行各阶段 AI 应该交付什么、不应该交付什么
- README 中教程入口同步改为新定位说明

验证方式：

- 直接检查 `docs/08-ai-prompt-tutorial.md`：结构已分为用户操作流程、提示词目标与实例、AI 交付物清单
- `git diff --check`
